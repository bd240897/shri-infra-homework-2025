...
## Адрес машины где будет развернут сервис
[http://158.160.168.111:3000/hw/store/](http://158.160.168.111:3000/hw/store/)

## Секреты
Были использованы следующие секреты для workflow

#### Для yandex cloud:
```bash
YANDEX_OAUTH_TOKEN - токен доступа в Yandex Cloud
YANDEX_REGISTRY_ID - идентификатор реестра docker репозитория в Yandex Cloud
```

#### Для виртуальной машины:
```bash
HOST - адрес машины (IP)
USERNAME - имя пользователя под которым заходим на машину
PRIVATE_KEY - приватный ключ, для входа на машину
``` 
